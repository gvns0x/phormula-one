...
export function createCar(world, startPos, startRotationY) {
  ...
  function applyInput(steer, throttle, brake, dt, reverse = 0, drsBoost = 0) {
    ...
    const drsOn = drsBoost > 0;
    const maxSpeed = ((tuning.maxSpeed ?? 0) * (1 + drsBoost)) / 3.6;
    const reverseMaxSpeed = maxSpeed * 0.3;
    bodyPhys.linearDamping = drsOn ? tuning.linearDamping * 0.2 : tuning.linearDamping;
    ...
    if (throttle > 0 && speed < maxSpeed) {
      const accelMul = drsOn ? 1.35 : 1;
      const acc = (engineForce / MASS) * acceleration * throttle * dt * accelMul;
...
const RIVAL_COLOR = 0x0055cc;

export function createRivalCar(world, startPos, startRotationY) {
  const group = new THREE.Group();
  ...
  const bodyMat = new THREE.MeshStandardMaterial({ color: RIVAL_COLOR });
  const cabinMat = new THREE.MeshStandardMaterial({ color: 0x111111 });
  ...
  const proceduralMeshes = [bodyMesh, cabinMesh];
  ...
  function applyInput(steer, throttle, brake, dt, reverse = 0, drsBoost = 0) {
    ...
    const speed = bodyPhys.velocity.dot(forward);
    lastSpeed = speed;
    gearbox.update(Math.abs(speed) * 3.6);
  ...
  function loadModel(url) {
    const rivalMat = new THREE.MeshStandardMaterial({ color: RIVAL_COLOR });
    const loader = new GLTFLoader();
    loader.load(
      url,
      (gltf) => {
      ...
      undefined,
      (err) => {
        console.warn('Rival car model failed to load, using procedural fallback:', err);
      }
    );
  }
  ...
  function getSpeed() { return lastSpeed; }
